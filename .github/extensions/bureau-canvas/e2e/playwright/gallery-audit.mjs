 ...
 * did not do the work, and a run that reports it has published an artefact that
 * lies about its own extent. `claims` compares two renders against each other,
 * and every claim requires both of them to have *proved* they stopped changing
 * first — so the drift this split was once made to tolerate is filtered out
 * before a claim exists, and both lists gate. `drift` is the subset that has
 * already said in its own words that the difference is a frame: it is the pairs
 * `undeclared` and `parted` refused to judge, and it is reported, never
 * asserted, because there is nothing in the product to go and look at.
 *

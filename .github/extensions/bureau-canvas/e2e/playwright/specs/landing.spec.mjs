...
test.describe("assignment landing", () => {
  ...
  test("a collapsed card summarises its source, repo count and limit count", async ({ page, canvas }) => {
    ...
    await expect(page.locator(".assignment-glance")).toHaveText("TheLarkInn/bureau · 1 repo · 2 limits");
  });
  ...
  test("opening the relation graph renders the config cards", async ({ page, canvas }) => {
    ...
    await expect(page.locator(".relation-section .card--assignment")).toHaveCount(1);
    await expect(page.locator(".relation-section .card--role")).toHaveCount(2);
  });

...
async fn fresh_state(ctx: &mut RunCtx, wt: &WtCtx, group: &StepDef) -> State {
    ...
    context::append(ctx, EventKind::GroupStarted, data);
...
fn retryable<'a>(
    ...
    context::append(ctx, EventKind::GroupMemberCancelled, data);
    None
}
...
fn start_available(
    ...
    for name in schedule.fill(ctx, &wt.mirror, root, &state.snapshot) {
        ...
        *attempt = attempt.saturating_add(1);
        ...
        context::append(ctx, EventKind::GroupMemberStarted, data);
    }
}
...
fn finish_member(
    ...
    context::append(ctx, EventKind::GroupMemberFinished, data);
...
pub(super) async fn run(ctx: &mut RunCtx, wt: &WtCtx, group: &StepDef) -> Execution {
    ...
    let mut schedule = schedule::new(members, state.limit, state.completion);
    ...
    context::append(ctx, EventKind::GroupFinished, data);
    execution
}

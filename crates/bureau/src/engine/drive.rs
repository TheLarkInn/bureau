...
fn fresh_open(
    ...
    let mut log = Appender::create(runs_dir, &plan.run_id, secrets, clock)
    ...
    Ok(Open::Running(Box::new(RunCtx::new(
        plan, log, history, clock,
    ))))
}
...
fn resume_ctx(
    ...
    let mut log =
        Appender::resume(dir, secrets, clock).map_err(|e| format!("opening run log: {e}"))?;
    if !history.started {
        append_started(&mut log, plan)?;
    }
    Ok(Open::Running(Box::new(RunCtx::new(
...
fn resume_open(
    ...
    match resume::replay(events, &plan.pipeline) {
        ...
        resume::Replay::Resume(history) => resume_ctx(dir, plan, secrets, history, clock),
    }
}

/// Opens the run fresh or resumes it from its event log.
fn open(dir: &Path, runs_dir: &Path, plan: &RunPlan, clock: fn() -> u64) -> Result<Open, String> {
    let secrets: Vec<Secret> = plan.credentials.values().cloned().collect();
    if dir.join(runlog::EVENTS_FILE).exists() {
        resume_open(dir, plan, &secrets, clock)
    } else {
        fresh_open(runs_dir, plan, &secrets, clock)
    }
}
...
async fn clear_stale(mirror: &Path, wt_dir: &Path, branch: &str, clock: fn() -> u64) {
    let dir = wt_dir.to_string_lossy().into_owned();
    let _ = gitcmd::git(&["worktree", "remove", "--force", &dir], mirror, &[], clock).await;
    let _ = gitcmd::git(&["worktree", "prune"], mirror, &[], clock).await;
    let _ = gitcmd::git(&["branch", "-D", branch], mirror, &[], clock).await;
}
...
async fn worktree_phase(cache: &CheckoutCache, ctx: &RunCtx) -> Result<WtCtx, String> {
    ...
    let worktree = Worktree::create(&mirror, &wt_dir, &branch, false, ctx.clock)
        .await
        .map_err(|e| format!("creating worktree failed: {e}"))?;
    let start_head = gitcmd::git(&["rev-parse", "HEAD"], worktree.path(), &[], ctx.clock)
        .await
...
pub(super) async fn run(
    ...
    match open(&dir, runs_dir, plan, clock) {

...
pub(super) fn validate(snapshot: &Path, config: &Path) -> Result<(), Error> {
...
pub(super) fn load_agent_files(
    ...
    roles
        .iter()
        .filter(|(_, role)| {
            role.adapter != crate::adapters::AdapterKind::Fake
                && !crate::plugin::is_plugin_reference(&role.agent)
        })
...
fn direct_path(config: &Path, value: &str) -> Result<std::path::PathBuf, Error> {
    ...
    if unsafe_path {
        return Err(Error::UnsafeConfig(value.to_owned()));
    }
    Ok(config.join(path))
}

fn validate_named_dir(snapshot: &Path, directory: &Path) -> Result<(), Error> {
...
fn require_dir(snapshot: &Path, path: &Path) -> Result<(), Error> {
    validate_optional(snapshot, path, true)?;
    Ok(())
}

fn validate_optional(snapshot: &Path, path: &Path, directory: bool) -> Result<bool, Error> {
...
fn unsafe_path(path: &Path) -> Error {
    Error::UnsafeConfig(path.display().to_string())
}

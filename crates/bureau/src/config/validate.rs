...
use super::validate_pipeline::{path_of, push};
...
fn check_repo_refs(
    ...
    if a.repos.is_empty() {
        push(errors, path, format!("assignment `{name}` lists no repos"));
    }
    for repo in &a.repos {
...
pub fn validate(config: &Config) -> Vec<ConfigError> {
    ...
    errors.extend(super::validate_pipeline::validate_pipelines(config));
    errors
}

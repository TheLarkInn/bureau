...
use super::files::{AdapterKind, Assignment, Role};
...
pub(super) fn push(errors: &mut Vec<ConfigError>, path: &Path, message: String) {
    errors.push(ConfigError::new(path, message));
}
...
fn check_step_names(errors: &mut Vec<ConfigError>, name: &str, pipeline: &Pipeline, path: &Path) {
    let mut err = |detail: &str| {
        push(errors, path, format!("pipeline `{name}` {detail}"));
    };
